...
class BoboSetupSimple(BoboSetup):
    ...
    def __init__(self,
        ...
        self._event_id_gen: BoboGenEventID = event_id_gen \
            if event_id_gen is not None else BoboGenEventIDUnique()
        self._event_gen: BoboGenEvent = event_gen \
            if event_gen is not None else BoboGenEventTime(
                milliseconds=1000,
                datagen=lambda: None)
        self._run_id_gen: BoboGenEventID = run_id_gen \
            if run_id_gen is not None else BoboGenEventIDUnique()
