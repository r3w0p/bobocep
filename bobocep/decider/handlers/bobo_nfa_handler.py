...
class BoboNFAHandler(AbstractHandler, IRunSubscriber):
    ...
    def __init__(self,
                 nfa: BoboNFA,
                 buffer: SharedVersionedMatchBuffer) -> None:
        super().__init__()
    ...
    def process(self, event: BoboEvent) -> None:
        with self._lock:
            self._check_event_against_runs(event)

            if self.nfa.start_state.process(event, {}):
                self.on_run_clone(
    ...
    def on_run_final(self,
        ...
        with self._lock:
            if notify:
                self._notify_final(run_id, history)
    ...
    def _check_event_against_runs(self, event: BoboEvent) -> None:
        for run in tuple(self._runs.values()):
            run.process(event)
    ...
    def _notify_final(self, run_id: str, history: BoboHistory) -> None:
        for subscriber in self._subs:
