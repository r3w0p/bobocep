...
class BoboNFAHandler(AbstractHandler, IRunSubscriber):
    ...
    :param max_recents: The maximum number of recent CompositeEvent instances
                        generated by this handler to pass to runs.
                        Minimum of 1, defaults to 1.
    :type max_recents: int, optional
    """
    ...
    def __init__(self,
        ...
        self._runs = {}
        self._recents = []
        self._max_recents = max(1, max_recents)
        self._subs = []
        self._lock = RLock()
    ...
    def _add_recent(self, event: CompositeEvent):
        if len(self._recents) == self._max_recents:
            self._recents.pop()

        self._recents.insert(0, event)

        # ensure first has largest timestamp i.e. is most recent
        self._recents.sort(key=lambda e: e.timestamp, reverse=True)
